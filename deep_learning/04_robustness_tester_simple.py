...
def show_distorted_predictions(model, transform, device, X_test, y_test, class_names, n_samples=15):
    ...
    rows, cols = len(indices), len(distortions)
    ...
    plt.suptitle("Распознавание CNN с искажениями", fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def show_misclassified(model, transform, device, X_test, y_test, class_names, n_samples=10, batch_size=64):
    ...
    for i, idx in enumerate(selected):
        ...
        axes[i].text(0.5, -0.15, top3_text, transform=axes[i].transAxes, fontsize=8, family='monospace')
        axes[i].axis('off')
